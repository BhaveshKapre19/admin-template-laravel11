<x-layouts.admin>
    <x-slot:title>{{ isset($post) ? 'Edit Post' : 'Create New Post' }}</x-slot:title>
    
    <div class="bg-gradient-to-br from-gray-800 to-gray-900 rounded-2xl shadow-2xl p-8 max-w-4xl mx-auto" 
         x-data="{
             tags: {{ json_encode(old('tags', isset($post) ? $post->tags->pluck('name')->toArray() : [])) }},
             addTag(tag) {
             ...
             featuredImagePreview: {{ isset($post) && $post->featured_image ? json_encode(asset($post->featured_image)) : 'null' }},
             isUploading: false
         }">
        <!-- Header Section -->
        <div class="mb-10 text-center">
            <h2 class="text-4xl font-bold bg-gradient-to-r from-blue-400 to-purple-500 bg-clip-text text-transparent">
                <i class="fas fa-magic mr-3"></i>{{ isset($post) ? 'Refine Your Masterpiece' : 'Craft Your Masterpiece' }}
            </h2>
            <p class="mt-2 text-gray-400">{{ isset($post) ? 'Polish your knowledge sharing' : 'Share your knowledge with the world' }}</p>
        </div>

        <form method="POST" action="{{ isset($post) ? route('posts.update', $post) : route('posts.store') }}" enctype="multipart/form-data" class="space-y-10">
            @csrf
            @isset($post)
                @method('PUT')
            @endisset
                ...
                <div class="border-2 border-gray-600/50 rounded-xl overflow-hidden hover:border-blue-500/30 
                    ...
                    <textarea name="content" id="editor" rows="10" required
                              class="w-full bg-gray-700/50 px-6 py-4 text-gray-100 
                                     focus:ring-4 focus:ring-blue-500/30 focus:border-transparent">{{ old('content', $post->content ?? '') }}</textarea>
                </div>
                @error('content')
                    <p class="text-red-400 text-sm mt-2 animate-pulse">
                        <i class="fas fa-exclamation-circle mr-1"></i>{{ $message }}
                ...
                <div class="bg-gray-700/30 p-6 rounded-xl border-2 border-gray-600/50 
                    ...
                    <textarea name="excerpt" rows="3" maxlength="300"
                              class="w-full bg-gray-800/20 rounded-lg px-4 py-3 text-gray-100 
                                     ...
                                     transition-all duration-300">{{ old('excerpt', $post->excerpt ?? '') }}</textarea>
                ...
                <div class="bg-gray-700/30 p-6 rounded-xl border-2 border-gray-600/50 
                    ...
                    <div class="relative">
                        <select name="status" required
                            ...
                            <option value="draft" class="bg-gray-800" @selected(old('status', $post->status ?? 'draft') == 'draft')>Draft</option>
                            <option value="published" class="bg-gray-800" @selected(old('status', $post->status ?? 'draft') == 'published')>Published</option>
                        </select>
            ...
            <div class="bg-gradient-to-br from-gray-700/30 to-gray-800/30 rounded-xl p-8 border-2 border-gray-600/50
                ...
                <div class="space-y-6">
                    ...
                    <div class="space-y-2">
                        ...
                        <input type="text" name="meta[title]" value="{{ old('meta.title', optional($post->meta ?? null)->title) }}"
                    ...
                    <div class="space-y-2">
                        ...
                        <textarea name="meta[description]" rows="2"
                                  class="w-full bg-gray-800/20 border-2 border-gray-600/50 rounded-lg px-4 py-3 
                                         text-gray-100 focus:ring-4 focus:ring-blue-500/30 transition-all duration-300">{{ old('meta.description', optional($post->meta ?? null)->description) }}</textarea>
                    </div>
                    ...
                    <div class="space-y-2">
                        ...
                        <input type="text" name="meta[keywords]" value="{{ old('meta.keywords', optional($post->meta ?? null)->keywords) }}"
                               class="w-full bg-gray-800/20 border-2 border-gray-600/50 rounded-lg px-4 py-3 
                                      text-gray-100 focus:ring-4 focus:ring-blue-500/30 transition-all duration-300"
            ...
            <div class="pt-8">
                <button type="submit"
                    ...
                    {{ isset($post) ? 'Update Post' : 'Launch Post' }}
                </button>
            </div>
        </form>
    </div>
